...
`npm publish --registry`. It also uses `tar` to repack tarballs after metadata
normalization. It does not need internet access and does not run `npm pack`.
...
By default, the offline publisher rewrites each tarball's
`package/package.json` before upload. It removes `scripts` and
`devDependencies`, then repacks with `tar`; this prevents `prepare`, build,
test, install, and postinstall lifecycle metadata from requiring packages that
are not present in the air-gapped environment. It keeps runtime import metadata
and dependencies, including `main`, `module`, `types`, `exports`,
`dependencies`, `peerDependencies`, and `optionalDependencies`.

If npm install from Artifactory is still blocked by registry resolution of peer
or optional packages that you intentionally did not mirror, publish with:
...
Only use those strip flags when the missing peer or optional packages are not
required by your runtime. Regular `dependencies` are never removed by the
normalizer because those are needed for the package to import correctly.
...
- `upload-npm-tarballs-to-artifactory.sh` for Artifactory publishing.

The upload script is intentionally separate. Artifactory enforces npm repository
rules, such as rejecting an already-published `name@version`, but the caller
still controls the dist-tag sent with `npm publish`. To avoid lower versions or
prerelease versions trying to take `latest`, the uploader publishes only the
highest non-prerelease version of each package with the `latest` dist-tag. Older
versions and prerelease versions use a unique `gitlab-mirror-<version>` tag.
...
- `NORMALIZE_LIBRARY_PACKAGE=true`: rewrite tarball package metadata before
  upload. This is the default. Set to `false` to publish original tarballs.
- `STRIP_PEER_DEPENDENCIES=true`: remove `peerDependencies` and
  `peerDependenciesMeta` during normalization.
- `STRIP_OPTIONAL_DEPENDENCIES=true`: remove `optionalDependencies` during
  normalization.
...
When `DRY_RUN` is not enabled, the mirror script writes
`gitlab-npm-upload-manifest.tsv` in `WORK_DIR` after packing tarballs from
GitLab, then calls:

```bash
./npm/upload-npm-tarballs-to-artifactory.sh \
  --manifest "$WORK_DIR/gitlab-npm-upload-manifest.tsv" \
  --registry-url "$ARTIFACTORY_NPM_REGISTRY" \
  --work-dir "$WORK_DIR/artifactory-upload" \
  --token "$ARTIFACTORY_TOKEN" \
  --skip-existing \
  --normalize-library-package
```

You can rerun that upload step directly if the GitLab download/pack phase
already succeeded. The standalone uploader uses the same library normalization
as the PowerShell bundle publisher, so it does not need to run `npm pack` in the
air-gapped environment.

To test the Artifactory upload path against a real npm-capable Artifactory
instance:
...
ARTIFACTORY_PASSWORD=password \
  ./npm/tests/integration-artifactory-npm.sh
```

The integration test creates/uses `npm-local-test`, publishes out-of-order
fixture package versions through `upload-npm-tarballs-to-artifactory.sh`,
verifies `latest` points only at the highest stable version, generates a
`package-lock.json` from Artifactory, and runs `npm ci` from that lockfile.
Artifactory OSS does not support npm repositories; the test exits `77` with a
skip message against OSS-only instances.
